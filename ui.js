...
const UI = {
  ...
  _boardHTML(VB, CR, CW, CH) {
    ...
    const cellsHTML = BOARD.map((cell, i) => {
      ...
      const pawnColor = isLight ? '#e63030' : '#ff4040';
      const pawn = isCur
        ? '<circle cx="' + cx2 + '" cy="' + cy2 + '" r="' + Math.round(minD*0.22) + '" fill="' + pawnColor + '" stroke="white" stroke-width="2.5" opacity="0.9"/>'
        : '';
  ...
  showCardResult(correct, pts, cb) {
    document.getElementById('r2-result-text').textContent = correct ? '+' + pts + ' pontos! 🎉' : 'Resposta errada. Continuem!';
...
function uiRevealAnswer() {
  ...
  correct.textContent = '✓ Certa';
  correct.onclick     = () => r2QuestionAnswer(true);
  const wrong = document.createElement('button');
  wrong.className   = 'btn-wrong';
  wrong.textContent = '✗ Errada';
  wrong.onclick     = () => r2QuestionAnswer(false);
